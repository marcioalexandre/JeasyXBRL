...
import linkbase.Arc;
import linkbase.Label;
import linkbase.Locator;
...
public class XbrlElement{
		...
		for(String i: instances){
			...
			while ((currentLine	= br.readLine()) != null){
				...
				if (m.find()){
					...
					//get Company
					if (ele.tagName.equals("dei:EntityRegistrantName")){
						ele.company = ele.value.trim();
					}else{	}
	...
	/*
	 * This method uses XML DOM connection, but
	 * it shows a very slow performance, that's whay I choose the another way
	 * which you can see in the others methods 
	...
	public void printElements(ArrayList<ArrayList<XbrlElement>> eles){
		...
		for (ArrayList<XbrlElement> eleList: eles){
			for (XbrlElement ele: eleList){
					i++;
					System.out.println("");
					System.out.println("Elemento number: "+ele.position);
					System.out.println("File name: "+ele.getFile_name());
					System.out.println("XML line: "+ele.getXml_line());
